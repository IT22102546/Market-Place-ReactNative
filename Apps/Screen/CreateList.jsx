...
import {
  ...
  where,
  getDocs,
} from "firebase/firestore"; // Firestore import
...
export default function CreateList() {
  ...
  const startVoiceCommand = () => {
    if (Platform.OS !== "web") {
      Alert.alert(
        "Sorry!",
        "Voice commands are only supported in web platforms currently."
      );
      return;
    }
  ...
  const processVoiceCommand = (command) => {
    ...
    if (lowerCaseCommand.startsWith("add")) {
      ...
      if (itemDetails) {
        ...
        Alert.alert(
          "No item specified",
          'Please say the item and quantity, e.g., "add sugar 1kg".'
        );
      }
    }
    // Handle "remove" command (e.g., "remove milk")
      ...
      if (itemToRemove) {
        ...
        Alert.alert(
          "No item specified",
          'Please say the item to remove, e.g., "remove milk".'
        );
      }
    }
  ...
  const removeItemFromList = (itemName) => {
    ...
    if (updatedList.length === shoppingList.length) {
      ...
      Alert.alert(
        "Success",
        `"${itemName}" has been removed from your shopping list.`
      );
    }
  };
    ...
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        if (data.list && Array.isArray(data.list)) {
          ...
          Alert.alert(
            "Error",
            "List is missing or improperly structured in Firebase."
          );
          return;
        }
      });
  ...
  const saveListToFirebase = async () => {
    if (shoppingList.length === 0) {
      Alert.alert(
        "Your list is empty",
        "Please add items to your list before saving."
      );
      return;
    }
  ...
  const findNearbyShops = async () => {
    try {
      ...
      if (shopList.length > 0) {
        Speech.speak(
          "Nearby shops found. You can select a shop from the list."
        );
